...
parser.add_argument("--infer_data_path", type=str, required=True, help="the path to the eval text file")
...
# %%
# parse the text file
with open(args.infer_data_path, "r") as f:
    lines = f.readlines()
...
with open('infer_result/result_'+ os.path.split(args.infer_data_path)[1], 'w+') as f:
      f.write('F1_MACRO = ' + str(f1_score(cut_notCut_label_all_gt, cut_notCut_label_all_trans, average="macro")))
      f.write('\n\n')

# %%
print(classification_report(cut_notCut_label_all_gt, cut_notCut_label_all_trans, digits=4))
with open('infer_result/result_'+ os.path.split(args.infer_data_path)[1], 'a') as f:
      f.write(str(classification_report(cut_notCut_label_all_gt, cut_notCut_label_all_trans, digits=4)))
      f.write('\n\n')

for t in transformer_output:
    sum_length = sum_length + len(t)
    with open('infer_result/result_'+ os.path.split(args.infer_data_path)[1], 'a') as f:
      f.write('['+','.join('\''+str(sentense)+'\'' for sentense in t)+']')
      f.write('\n')
      f.write('LENGTH = '+ str(len(t)))
      f.write('\n\n')

with open('infer_result/result_'+ os.path.split(args.infer_data_path)[1], 'a') as f:
      f.write('SUM LENGTH = '+str(sum_length))
      f.write('\n\n')